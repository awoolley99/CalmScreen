...
import { getDatabaseShowAnalysis, searchDatabaseShows } from './supabaseShows';
...
export async function analyzeShow(title: string): Promise<ShowAnalysis> {
  const databaseAnalysis = await getDatabaseShowAnalysis(title);
  if (databaseAnalysis) {
...
export async function searchShows(query: string): Promise<string[]> {
  const databaseResults = await searchDatabaseShows(query);
